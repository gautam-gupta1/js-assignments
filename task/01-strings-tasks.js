...
function getRectangleString(width, height) {
       let UpperWidthString = "┌" + "─".repeat(width-2)+"┐"+ "\n";
       let heightString = "│" + " ".repeat(width-2) + "│"+"\n";
       let LowerWidthString = "└" + "─".repeat(width-2)+"┘"+"\n";
    return UpperWidthString + heightString.repeat(height-2) + LowerWidthString;
}
...
function encodeToRot13(str) {
    ...
    const ansArr = (str.split("")).map((chr,index)=>{
      let asciiVal = chr.charCodeAt(0);
      if(asciiVal>=65&&asciiVal<=91){
         const newChr = (asciiVal-65 +13)%26 + 65;
         return String.fromCharCode(newChr);
      }
      else if(asciiVal>=97&&asciiVal<=123){
        const newChr = (asciiVal-97 +13)%26 + 97;
        return String.fromCharCode(newChr);
      }
      return chr;
...
function getCardId(value) {
    ...
    const cardNumber = value.slice(0,-1);
    const cardSymbol = value.slice(-1);
    let numberValue = Number(cardNumber);
    numberValue = isNaN(numberValue)?symbolToValue[cardNumber]: numberValue;
    return numberValue + symbolToValue[cardSymbol]-1;
}
