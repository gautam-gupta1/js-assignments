...
function getPolynom(...args) {
    ...
    return function(x){
        ...
        let ans =  args.reduce((arg,value,index)=>{
             return arg+ value*Math.pow(x,args.length-index-1);
...
function memoize(func) {
    ...
    return (...args)=>{
       ...
       let key = args.length==0?"emptyKey":args.join();
...
function partialUsingArguments(fn,...args) {
    ...
    return (...newArgs)=>{
        return args.join("").concat(newArgs.join(""));
    }
